...
def talleres_hoy(db: Session = Depends(get_db)):
    ...
    try:
        ...
        for taller in talleres_de_hoy:
            usuario_taller = db.query(UsuarioTaller).filter(
                UsuarioTaller.idTaller == taller.idTaller
            ).first()

            if usuario_taller:
                usuario = db.query(Usuario).filter(
                    Usuario.idUsuario == usuario_taller.idUsuario
                ).first()
                nombre_profesional = usuario.nombres, usuario.apellidos if usuario else "Desconocido"
            else:
        ...
        return {"message": "Talleres encontrados", "data": resultado}
...
def create_taller(taller: TallerCreate, db: Session = Depends(get_db)):
    try:
        ...
        db.rollback()
        print(f"Error en el servidor: {str(e)}")  # Agrega esta línea para registrar el error
...
def get_current_user(token: str = Depends(lambda: "access_token")):
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        numeroDocumento = payload.get("sub")
...
@router.get("/perfil")
def obtener_perfil(token: str = Depends(get_current_user), db: Session = Depends(get_db)):
    usuario = db.query(Usuario).filter(Usuario.numeroDocumento == token).first()
    if not usuario:
        raise HTTPException(status_code=404, detail="Usuario no encontrado")
    ...
    return {
        ...
        "claveUsuario": ""  # No devolvemos la contraseña por seguridad
    }
...
def actualizar_perfil(
    ...
    usuario.tipoDocumento = perfil.tipoDocumento
    usuario.numeroDocumento = perfil.numeroDocumento
    usuario.nombres = perfil.nombres
    usuario.apellidos = perfil.apellidos

    # Encriptamos la nueva contraseña si se proporcionó
    if perfil.claveUsuario:
        import bcrypt
        usuario.claveUsuario = bcrypt.hashpw(perfil.claveUsuario.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
